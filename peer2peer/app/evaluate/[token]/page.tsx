        ...
        <div style={{ width: 80 }}>
          ...
          >
            <div
              style={{
                height: "100%",
                width: `${(filled / criteria.length) * 100}%`,
...
export default function EvaluatePage() {
  ...
  // scores[peerId][criterionId] = score
  ...
  const totalCriteria = data
    ? data.peers.length * data.evaluation.criteria.length
    : 0;
  ...
  const totalFilled = data
    ? data.peers.reduce(
        (sum, peer) =>
          sum +
          data.evaluation.criteria.filter(
            (c) => scores[peer.id]?.[c.id] !== undefined
          ).length,
        0
      )
    : 0;
  ...
  async function handleSubmit() {
    ...
    try {
      const responses: { evaluatedId: number; criterionId: number; score: number }[] = [];
      ...
      <div style={cardStyle}>
          ...
          >
            ...
            {data.evaluation.deadline && (
              <div style={{ ...pillStyle, color: "#b45309", background: "#fffbeb" }}>
                ⏰ Due:{" "}
                {new Date(data.evaluation.deadline).toLocaleDateString("en-US", {
                  month: "short",
                  day: "numeric",
                  year: "numeric",
                })}
            ...
            {data.evaluation.anonymous && (
              <div style={{ ...pillStyle, color: "#7c3aed", background: "#f5f3ff" }}>
                🔒 Anonymous
              </div>
            )}
          </div>
        ...
        >
          ...
          >
            ...
            <span style={{ fontWeight: 600, color: "#111827" }}>
              {totalFilled} / {totalCriteria} scores
            </span>
          </div>
          <div
            style={{
        ...
        {data.peers.map((peer, i) => (
          <PeerCard
            ...
            onScore={(criterionId, score) => handleScore(peer.id, criterionId, score)}
            index={i}
            isActive={activePeerId === peer.id}
            onActivate={() =>
              setActivePeerId((prev) => (prev === peer.id ? null : peer.id))
            }
          />
        ))}
