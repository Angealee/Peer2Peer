...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    for (const response of evaluation.responses) {
      ...
      const commentKey = `${response.evaluatorStudentId}-${response.evaluatedStudentId}`;
      if (response.comment && response.comment.trim() && !commentTracker.has(commentKey)) {
        commentTracker.set(commentKey, true);
        entry.comments.push({
          evaluatorName: isAnonymous ? "Anonymous" : response.evaluatorStudent.name,
          text: response.comment.trim(),
        });
      }
    }

    const results = Array.from(studentMap.values()).map(({ student, scores, comments }) => {
      ...
      const overallAverage =
        scoreArray.length > 0
          ...
          : 0;
      return { student, scores: scoreArray, overallAverage, comments };
    });

    return NextResponse.json({ evaluationId, title: evaluation.title, results });
  } catch (err) {
